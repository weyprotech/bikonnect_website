            ...
            <li><a href="javascript:;">{{ trans('lang.products') }}</a>
                <ul>
                    @foreach($productList as $productKey => $productValue)
                        <li><a href="{{ route('main.product',[$productValue->Id,app()->getLocale()]) }}">{{ $productValue->lang[0]->title }}</a></li>
                    @endforeach
                </ul>
            </li>
            <li><a href="{{ route('main.about',app()->getLocale()) }}">{{ trans('lang.aboutus') }}</a></li>
